...
export default function AppTabs({ active }: { active?: Tab } = {}) {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-black/10 bg-white/90 pb-[env(safe-area-inset-bottom)] backdrop-blur-xl">
      <div className="mx-auto flex max-w-5xl items-stretch justify-around px-6 py-1">
        <TabLink
          href="/"
          label="Library"
          icon={<LibraryIcon />}
...
function TabLink({
  ...
  return (
    <Link
      href={href}
      className={`flex min-w-[5rem] flex-col items-center gap-px rounded-lg px-4 py-1 text-[10px] font-medium transition ${
